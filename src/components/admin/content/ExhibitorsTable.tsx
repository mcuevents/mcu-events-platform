        ...
        <table className="w-full text-left text-xs border-collapse">
          <thead>
            <tr className="border-b border-dark-800 bg-dark-950/60 text-[10px] font-bold uppercase tracking-wider text-dark-400">
              ...
              <th className="py-3.5 px-3 font-semibold">Exhibitor & Brand</th>
              <th className="py-3.5 px-3 font-semibold">Stall / Booth</th>
              <th className="py-3.5 px-3 font-semibold hidden md:table-cell">Associated Event</th>
              <th className="py-3.5 px-3 font-semibold hidden lg:table-cell">Stall Contact</th>
          ...
          <tbody className="divide-y divide-dark-800/60">
            {items.length === 0 ? (
              <tr>
                <td colSpan={7} className="py-16 text-center">
                  <div className="flex flex-col items-center justify-center space-y-2">
                    ...
                    <p className="text-sm font-bold text-white">No exhibitors configured</p>
                    <p className="text-xs text-dark-400 max-w-sm">
                      Add participating brands and allocate stall booth numbers for upcoming expos.
                    </p>
              ...
              items.map((ex) => (
                <tr key={ex.id} className="hover:bg-dark-950/40 transition-colors group">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0 max-w-sm">
                        ...
                        {ex.websiteUrl && (
                          ...
                          >
                            <span>{ex.websiteUrl.replace(/^https?:\/\//, '')}</span>
                  ...
                  <td className="py-3.5 px-3 whitespace-nowrap">
                    {ex.boothNumber ? (
                      <span className="px-2.5 py-1 rounded-lg bg-dark-950 border border-dark-800 font-mono text-brand-400 font-bold text-[11px]">
                        {ex.boothNumber}
                      </span>
                    ) : (
                      <span className="text-dark-500 text-[11px]">Unassigned</span>
                    )}
                  </td>
                  ...
                  <td className="py-3.5 px-3 hidden md:table-cell">
                    {ex.eventTitle ? (
                      ...
                      <span className="text-dark-500 text-[11px]">General Directory</span>
                    )}
                  </td>
                  ...
                  <td className="py-3.5 px-3 hidden lg:table-cell">
                    <div className="space-y-0.5 max-w-[160px]">
                      {ex.contactPerson && (
                        <span className="text-dark-300 font-medium truncate block">
                          {ex.contactPerson}
                        </span>
                      )}
                      {ex.contactPhone && (
                        <span className="text-[10px] text-dark-400 flex items-center gap-1 font-mono">
                  ...
                  <td className="py-3.5 px-4 text-right whitespace-nowrap">
                    <div className="flex items-center justify-end gap-1">
                      <Button
                        variant="ghost"
                        size="sm"
                        className={`h-7 w-7 p-0 ${ex.isActive ? 'text-amber-400' : 'text-emerald-400'}`}
                        onClick={() => onToggleActive(ex.id, !ex.isActive)}
                        title={ex.isActive ? 'Deactivate exhibitor' : 'Activate exhibitor'}
                        disabled={isActionLoading}
                      >
                        {ex.isActive ? <EyeOff className="h-3.5 w-3.5" /> : <Eye className="h-3.5 w-3.5" />}
                      </Button>
